...
SpiLcdDisplay::SpiLcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
    ...
    if (psram_size_mb >= 8) {
        lv_img_cache_set_size(2 * 1024 * 1024);
        ESP_LOGI(TAG, "Use 2MB of PSRAM for image cache");
    } else if (psram_size_mb >= 2) {
        lv_img_cache_set_size(512 * 1024);
    ...
    if (offset_x != 0 || offset_y != 0) {
        // lv_display_set_offset not available in LVGL v8
    }
...
void LcdDisplay::ExitLockScreen() {
    ...
    lv_obj_add_flag(lock_screen_, LV_OBJ_FLAG_HIDDEN);
    lv_obj_clear_flag(lock_screen_, LV_OBJ_FLAG_CLICKABLE);
    if (lock_btn_) lv_obj_clear_flag(lock_btn_, LV_OBJ_FLAG_HIDDEN);
}
...
void LcdDisplay::SetPreviewImage(const lv_img_dsc_t* img_dsc) {
    ...
    if (img_dsc != nullptr) {
        ...
        lv_obj_t* preview_image = lv_img_create(img_bubble);
        ...
        lv_img_set_src(preview_image, copied_img_dsc);
        lv_img_set_zoom(preview_image, zoom);
...
void LcdDisplay::SetupUI() {
    ...
    preview_image_ = lv_img_create(content_);
...
void LcdDisplay::UpdateStatusBar(bool update_all) {
    ...
    // On Smart Home tile (col 2), show page title instead of clock
    if (tileview_ != nullptr) {
        lv_obj_t *act = lv_tileview_get_tile_act(tileview_);
        if (act != nullptr && lv_obj_get_index(act) == 2) {
            lv_label_set_text(status_label_, "Smart Home Control");
        }
    }
    ...
    if (content_ != nullptr) {
        ...
        for (uint32_t i = 0; i < child_count; i++) {
            ...
            if (bubble_type_ptr != nullptr) {
                ...
                // 检查用户bubble
                if (bg_color.full == DARK_USER_BUBBLE_COLOR.full || 
                    bg_color.full == LIGHT_USER_BUBBLE_COLOR.full ||
                    bg_color.full == current_theme_.user_bubble.full) {
                    is_user_bubble = true;
                }
                // 检查系统bubble
                else if (bg_color.full == DARK_SYSTEM_BUBBLE_COLOR.full || 
                         bg_color.full == LIGHT_SYSTEM_BUBBLE_COLOR.full ||
                         bg_color.full == current_theme_.system_bubble.full) {
                ...
                if (lv_obj_get_child_cnt(bubble) > 0) {
                    ...
                    if (text != nullptr) {
                        // 回退到颜色检测逻辑
                        if (bg_color.full == current_theme_.system_bubble.full ||
                            bg_color.full == DARK_SYSTEM_BUBBLE_COLOR.full || 
                            bg_color.full == LIGHT_SYSTEM_BUBBLE_COLOR.full) {
                            lv_obj_set_style_text_color(text, current_theme_.system_text, 0);
                        } else {
                            lv_obj_set_style_text_color(text, current_theme_.text, 0);
...
void LcdDisplay::HandleEc11KeyEvent(bool is_press) {
    ...
    if (ec11_mode_ == Ec11Mode::NAV) {
        if (ec11_focused_ != nullptr) {
            lv_event_send(ec11_focused_, LV_EVENT_CLICKED, nullptr);
        } else {
