        ...
        for (int i = 0; i <= lv_flie.list_flie_nuber ;i++)  /* 轮询列表项 */
        {
            ...
            {   
                ...
                {   
                    ...
                    lv_scan_files(lv_flie.pname, lv_flie.file_main_ui);          /* 重新创建文件列表 */
                }
                else
                {
...
void lv_back_btn_event_handler(lv_event_t *event)
{
    ...
    if(code == LV_EVENT_SHORT_CLICKED)
    {
        ...
        {
            lv_del_list(lv_flie.list);      /* 删除列表 */
            lv_obj_del(lv_flie.file_main_ui); /* 删除文件管理器主容器 */
            /* 这里可以添加返回到主界面的逻辑 */
            lv_general.current_parent = lv_scr_act(); /* 设置当前父对象为屏幕 */
        }
        ...
        { 
            ...
            lv_scan_files(lv_flie.lv_prev_file[lv_flie.lv_prev_file_flag], lv_flie.file_main_ui);   /* 读取文件路径 */
        }
    }
}
...
void app_file_ui_init(void)
{
    ...
    else 
    {
		...
		// lv_obj_set_style_bg_color(lv_flie.file_main_ui, lv_color_hex(0x000000), LV_STATE_DEFAULT);                  /* 设置背景颜色 */
		lv_obj_set_size(lv_flie.file_main_ui, lv_obj_get_width(lv_scr_act()), lv_obj_get_height(lv_scr_act()) - lv_obj_get_height(lv_scr_act()) / 20);  /* 设置容器大小 */
		lv_obj_set_style_radius(lv_flie.file_main_ui, 0, LV_STATE_DEFAULT);                                         /* 无圆角 */
		lv_obj_set_style_border_opa(lv_flie.file_main_ui, LV_OPA_0, LV_STATE_DEFAULT);                              /* 边界透明 */
		lv_obj_set_pos(lv_flie.file_main_ui, 0, lv_obj_get_height(lv_scr_act()) / 20);                              /* 设置位置 */
		lv_obj_clear_flag(lv_flie.file_main_ui, LV_OBJ_FLAG_SCROLLABLE);                                            /* 禁止滚动 */
		lv_obj_update_layout(lv_flie.file_main_ui);

		lv_page_tile(lv_flie.file_main_ui);         /* 设置页面标题 */
		lv_page_back(lv_flie.file_main_ui);         /* 设置页面返回 */
		list_init(lv_flie.file_main_ui);            /* 列表初始化 */

		lv_general.current_parent = lv_flie.file_main_ui; /* 设置当前父对象 */
	}
}