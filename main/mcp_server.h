...
public:
    ...
    Property(const std::string& name, PropertyType type, int min_value, int max_value)
        ...
        if (type != kPropertyTypeInteger) {
            ESP_LOGE("Property", "Range limits only apply to integer properties");
            return;
        }
    }
    ...
    Property(const std::string& name, PropertyType type, int default_value, int min_value, int max_value)
        ...
        if (default_value < min_value || default_value > max_value) {
            ESP_LOGE("Property", "Default value must be within the specified range");
            return;
        }
        value_ = default_value;
    }
    ...
    inline void set_value(const T& value) {
        ...
        if constexpr (std::is_same_v<T, int>) {
            if (min_value_.has_value() && value < min_value_.value()) {
                ESP_LOGE("Property", "Value is below minimum allowed");
                return;
            }
            if (max_value_.has_value() && value > max_value_.value()) {
                ESP_LOGE("Property", "Value exceeds maximum allowed");
                return;
            }
        }
        value_ = value;
...
public:
    ...
    const Property& operator[](const std::string& name) const {
        ...
        ESP_LOGE("Property", "Property not found: %s", name.c_str()); static Property dummy("", kPropertyTypeString); return dummy;
    }
